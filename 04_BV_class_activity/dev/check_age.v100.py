...
def main():
    ...
    for v in versions_to_run:
        ...
        for age in unique_ages:
            ...
            if v == 0:
                if age > 100:
                    approved = True
            elif v == 1:                # This is the correct implementation
                if age < 100:
                    approved = True
            elif v == 2:
                if age != 100:
                ...
                # This code version simulates the case where the age check is incorrectly coded as : "if age := 100"
                # The error sets 'age' to be 100, regardless to the input, so it's alway accepted. 
                # If I actually put this code line in the script (and not commented out), it actually changes to input 
                # age values to 100, for all inputs, and then the output is strange (prints 100 as if these were all the inputs)  
                approved = True
            elif v == 6:
                if age == 100:
                    approved = True
    ...
    for v, accepted, rejected in table_rows:
        # We manually calculate padding lengths because ANSI escape codes alter len() calculations
