...
* **Teacher authentication — same mechanism as the reference app, unchanged:** a `Teachers` sheet allowlist, checked via `Session.getActiveUser().getEmail()` in `isAuthorizedTeacher_()`. An earlier draft of this doc claimed this couldn't work under an anonymous deployment and used a shared password instead — that claim turned out to be unverified speculation about Apps Script's platform behavior, contradicted by the reference app's own working code (a single deployment, no split, and `Student.html` even calls the same identity check to *prefill* students' email — gracefully falling back to blank rather than requiring it). Reverted to the identical mechanism per §7. **Still genuinely unverified for this app specifically** — see §7's honest caveat.
...
* **Access route:** `.../exec?role=teacher`, while signed into a Google account listed on the `Teachers` sheet (§5.6). Anything else — not signed in, signed into an unlisted account, or `role=teacher` alone with no match — transparently returns the ordinary student page instead, with nothing teacher-related in the response at all (no bug list, no settings markup to view-source). Every settings-changing action re-checks the same identity server-side, independent of how the page was reached (§7).
...
Google Sheet tabs, including `Teachers` (the allowlist — see §7):
...
#### 5.6 `Teachers`
The teacher allowlist — one email per row in column A, starting at row 2 (row 1 is a header, any text). Read only by `isAuthorizedTeacher_()` (§7); never created automatically (unlike every other sheet here) and never exposed to any client-facing function, matching the reference app's own `Teachers` sheet exactly. Add your own email, and any TA/co-instructor emails, here manually before using `?role=teacher`.
...
Copied from the reference app (`10 Combinatorial testing Google Web App requirements.md`, §7) as closely as possible, per an explicit preference for consistency over inventing a parallel mechanism.

* **History of this section, for transparency:** an earlier draft claimed `Session.getActiveUser()` "cannot reliably identify anyone" under this app's fully-anonymous deployment, and used a shared password instead of the reference app's `Teachers`-sheet allowlist. That claim was unverified speculation about Apps Script's platform internals, not something actually confirmed — and it doesn't survive contact with the reference app's own working code: a single deployment (no split), with `Session.getActiveUser()` relied on for **both** roles (`Student.html` calls it too, to prefill the email field, with a graceful no-op if it comes back blank). That's concrete evidence against the earlier claim, so the password mechanism has been removed and replaced with the identical allowlist approach.
* **`Teachers` sheet allowlist (§5.6), checked via `isAuthorizedTeacher_()`:** `doGet` serves `teacher.html` only when `role=teacher` **and** `Session.getActiveUser().getEmail()` is on that list; anything else — including a student who simply appends `?role=teacher` to the URL — transparently gets the ordinary student page, with no error or hint a teacher mode exists to try to bypass. This is the exact mechanism, not an adaptation.
* **Still genuinely unverified for this specific app, and worth saying plainly:** the reference app's institutional deployment and this one aren't proven identical in every relevant way. The only way to know for certain is to actually deploy this and check whether a signed-in, allowlisted visitor gets in. If not, this section needs revisiting — with an actual confirmed constraint next time, not a remembered one.
* **Server-side re-check on every privileged function, independent of the page-level gate:** `teacherGetSettings`, `teacherUpdateSettings`, and `teacherResetAll` all independently call `requireTeacher_()` — never trusted as a one-time check at page load (Apps Script exposes every top-level server function globally via `google.script.run`, regardless of which page called it, so this layer has to hold on its own). Matches the reference app's own `requireTeacher()` pattern.
* **Not reachable via the "under the UI" JSON API (§8):** a bare HTTP request has no signed-in browser session for `Session.getActiveUser()` to read, so teacher actions are only ever available through `teacher.html` itself — this is an inherent consequence of identity-based auth, not a gap.
