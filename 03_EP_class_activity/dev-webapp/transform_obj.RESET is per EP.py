...
def get_or_prompt_student_name():
    """Reads the cached student name or prompts for a new one if missing."""
    ...
    while True:
        try:
            name = input("Please enter your Student ID / Name for test tracking: ").strip()
            if name:
...
def main():
    # Detect and extract the --reset flag if present anywhere in the arguments
    is_reset_mode = False
    args = sys.argv[1:]
    if "--reset" in args:
        is_reset_mode = True
    ...
    # --- ER-9.1 & ER-9.2 & FR-14: Structural Argument Evaluation ---
    if len(args) == 0:
        print_usage()
        sys.exit(0)
    elif len(args) == 1:
    ...
    try:
        ...
        if '.' in raw_translation:
            ...
            print(f"[INFO] Translation input '{raw_translation}' given as a float. Rounded to nearest integer: {translation}.")
        else:
